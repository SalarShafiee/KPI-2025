    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            for i, (stage, actual, target) in enumerate(zip(stages, actual_values, target_values)):
                ...
                # Add engineering-style measurement for "ist"
                y_measurement_ist = y_bottom + 0.02  # Slightly above the shape but inside
                ax.plot(
                    [0.5 - width_actual_bottom, 0.5 + width_actual_bottom],
                    [y_measurement_ist, y_measurement_ist],
                    color="black",
                    linestyle="-",
                    linewidth=1
                )
                # Draw arrow tips for "ist"
                ax.annotate(
                    "", xy=(0.5 - width_actual_bottom, y_measurement_ist), xytext=(0.5 - width_actual_bottom - 0.02, y_measurement_ist),
                ...
                # Add engineering-style measurement for "soll"
                y_measurement_soll = y_top - 0.02  # Slightly below the dashed line, inside the shape
                ax.plot(
